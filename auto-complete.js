...
class AutoComplete extends LitElement {
  ...
  constructor() {
    ...
    this.renderOptions1 = this.renderOptions1.bind(this);
  ...
  onSearchTextChange(text) {
    this.filteredOptions = [
      ...this.options.filter((option) => {
        const suggestionName = option[this.displayBy].toLowerCase();
        const searchName = trimString(text).toLowerCase();
  ...
  addSelection(selectedOption) {
    if (this.isMultiple) {
      ...
      this.onChange(selectedOption[this.selectBy]); //changed here...
    }
  }
  ...
  removeSelection(selectionToRemove) {
    ...
    if (this.isMultiple) {
      ...
      this.selectedOptions = [
        ...this.selectedOptions,
        this.selectedOptions.filter(shouldNotBeRemoved),
      ];
  ...
  renderLabel() {
    ...
    return html` <label class="${labelClasses}"> ${this.label} </label> `;
  }
    ...
    const optionsMenu = html`
      ...
      >
        ${this.canCreateData && !isExactMatch && this.searchText
          ...
          : nothing}
        ${options.map(
          (option) => html` ${this.renderIndividualOption(option)} `
        )}
      </mwc-list>
    `;
